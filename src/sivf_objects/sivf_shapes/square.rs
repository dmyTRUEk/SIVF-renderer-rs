...
impl TraitRender for Square {
    ...
    fn render(&self, image_sizes: ImageSizes, render_type: RenderType) -> Canvas {
        ...
        let position: Vec2d<f64> = Vec2d::new(
            ...
            self.position.y.to_pixels(image_sizes.h)
        );
        let shift: Vec2d<f64> =
            - Vec2d::new(image_sizes.w as f64, image_sizes.h as f64)
            - position
            + Vec2d::new(0.5_f64, 0.5_f64);
        // TODO LATER: think about this - - - - - - - - - - - - - - - >  !W!
        let side: f64 = self.side.to_pixels(image_sizes.w);
        let side_half: f64 = side / 2_f64;
        let vec_min: Vec2d<f64> = position - Vec2d::new(side_half, side_half);
        let vec_max: Vec2d<f64> = position + Vec2d::new(side_half, side_half);
        match render_type {
            RenderType::Cpu1 => {
                for h in image_sizes.h.indices() {
                    for w in image_sizes.w.indices() {
                        let (x, y): (f64, f64) = (w as f64, h as f64);
                        let is_inside_figure: bool = Vec2d::new(x, y).is_inside_rectangle(vec_min, vec_max);
